...
It will train on the dataset for a 100 epochs, with around 5.5k observations per epoch. \
Afterwards it'll run a python script to create a graph of the loss over time.
...
The formula is: `smoothloss = 0.999 * smoothloss + 0.001 * loss`, and since the *training-set* is bigger, it'll have more batches to update the smooth loss with, so it'll change faster than *testing-set*.
...
The loss function for the graphs aren't standerdized
